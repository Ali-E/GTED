...
class Graph(object):
    ...
    def node_orders(self):
        ...
        return {x: self.edge_weights(x) for x in self.nodes}

    @property
    def node_orders_2(self):
    ...
    def odd_nodes(self):
        ...
        return [k for k in self.nodes if not \
                my_math.is_even(self.edge_weights(k))]
    ...
    def edge_weights(self, node):
        new_dict =  [x.contents[2] for x in self.edges.values() \
                    if node in (x.head, x.tail)]
        return sum(new_dict)
